...
class DownloaderApp(ctk.CTk):
    def __init__(self):
        ...
        self.columns_frame.pack(fill="both", expand=True, padx=30, pady=4)
        ...
        self.input_card = ctk.CTkFrame(self.left_column, fg_color="#0E1A24", corner_radius=12, border_color="#1F3A4E", border_width=1)
        self.input_card.pack(fill="both", expand=True, pady=2)
        ...
        self.quality_card = ctk.CTkFrame(self.right_column, fg_color="#0E1A24", corner_radius=12, border_color="#1F3A4E", border_width=1)
        self.quality_card.pack(fill="x", pady=(0, 4))
        ...
        lbl_vid = ctk.CTkLabel(video_frame, text="Video Quality", font=("Segoe UI", 10), text_color="#78909C")
        ...
        lbl_aud = ctk.CTkLabel(audio_frame, text="Audio Quality", font=("Segoe UI", 10), text_color="#78909C")
        ...
        lbl_thm = ctk.CTkLabel(theme_frame, text="App Theme", font=("Segoe UI", 10), text_color="#78909C")
        ...
        # --- Panel 3: Options ---
        self.options_card = ctk.CTkFrame(self.right_column, fg_color="#0E1A24", corner_radius=12, border_color="#1F3A4E", border_width=1)
        ...
        self.action_card = ctk.CTkFrame(self.right_column, fg_color="#0E1A24", corner_radius=12, border_color="#1F3A4E", border_width=1)
        self.action_card.pack(fill="x", pady=(4, 2))
        ...
        self.season_checkbox.pack(pady=(0, 2), anchor="c")
        ...
        self.season_input.pack(anchor="c")
        ...
        self.speed_lbl.pack(pady=(0, 2), anchor="c")
        ...
        self.speed_input.pack(anchor="c")
        ...
        # Queue label
        self.queue_label = ctk.CTkLabel(self.action_card, text="Queue: Ready", font=("Segoe UI", 11, "bold"), text_color="#78909C")
        self.queue_label.pack(pady=(0, 6))
        self.theme_labels_secondary.append(self.queue_label)

        # --- Footer Unified Control Toolbar ---
        self.footer_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.footer_frame.pack(fill="x", padx=30, pady=(8, 12))
        ...
        self.update_cookies_btn.grid(row=0, column=0, padx=(0, 4), sticky="ew")
        ...
        self.update_ytdlp_btn.grid(row=0, column=1, padx=(4, 4), sticky="ew")
        ...
        self.toggle_log_btn.grid(row=0, column=2, padx=(4, 4), sticky="ew")
        ...
        self.open_error_log_btn.grid(row=0, column=3, padx=(4, 0), sticky="ew")
        ...
        try:
            ...
            import comtypes.gen.TaskbarLib as tbl  # type: ignore
    ...
    def show_log(self):
        if not self.log_visible:
            ...
            # Update window height to 700 while preserving coordinates
            geom = self.geometry()
            if geom:
                match = re.match(r"\d+x\d+\+(\d+)\+(\d+)", geom)
                if match:
                    x, y = match.group(1), match.group(2)
                    self.geometry(f"860x700+{x}+{y}")
                else:
    ...
    def hide_log(self):
        if self.log_visible:
            ...
            # Update window height to 520 while preserving coordinates
            geom = self.geometry()
            if geom:
                match = re.match(r"\d+x\d+\+(\d+)\+(\d+)", geom)
                if match:
    ...
    def display_search_results(self, results):
        ...
        for idx, item in enumerate(results):
            ...
            result_card.grid(row=0, column=idx, padx=3, pady=4, sticky="nsew")
            ...
            if item["thumb_path"] and os.path.exists(item["thumb_path"]):
                try:
                    ...
                    pass
            if not img:
                img = Image.new('RGB', (110, 62), color='#1E1A24')
                
            ctk_img = ctk.CTkImage(light_image=img, dark_image=img, size=(110, 62))
